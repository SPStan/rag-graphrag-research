...
The runner's retry predicate now includes `finish_reason=length`, stage retry caps are explicit CLI settings included in the manifest and storage namespace fingerprint, and a unit test covers truncated output whose partial JSON was otherwise parseable. Producer identity is separated from run/storage policy so a retry-cap change creates a fresh namespace without making compatible upstream stores appear to come from a different model or embedding pipeline. A read-only audit verifies all 5,500 passage vectors, 44,614 entity vectors, 47,614 fact vectors, graph and 5,500-row OpenIE state. Producer, embedding and OpenIE identities, corpus SHA and exact vector IDs all match; no existing vector is missing or stale. It estimates 196 targeted generation calls if corrected NER outputs also refresh dependent triples. A repair can therefore clone the compatible stores and avoid full passage re-embedding, subject to implementing and testing clone/repair behavior. The default retry caps are a protocol proposal; inspect expected prompt lengths and confirm hardware/context feasibility before targeted calls. This ADR does not authorize or trigger repair calls, graph reconstruction, or QA. Before a candidate comparison, verify the frozen configuration, cache behavior, manifest fields, zero unresolved gate outcomes, and exact question-ID pairing. A failed gate means no HippoRAG QA and no paired metrics/bootstrap.
