...
- Date: 2026-09-25
- Status: accepted; attempt-level instrumentation and a candidate ID view are implemented; evaluation is not run
...
The runner now writes an attempt ledger and an acceptance-gate report to each new run manifest; synthetic tests cover status classification, retries and missing stage outcomes. A result returned only by HippoRAG's higher-level OpenIE result cache has no observed LLM-layer attempt and cannot pass the independent-index gate, even if the cached parsed value is non-empty or empty. A captured LLM-cache hit is attributable and records `cache_hit=true`. Historical runs are unchanged and have no reconstructed attempt ledger. Any rebuild still requires an explicit run command and a new fingerprinted storage namespace. Never overwrite or delete the current index, cache, or historical results. This ADR authorizes the instrumentation and ID-view preparation, not an LLM call, index rebuild or benchmark run.

## Independent evaluation plan (planned, not run)

Candidate evaluation IDs are positions 200–299 of the pinned `data/ids/musique_s500.json` list (`S500[200:300]`, 100 questions). The frozen, not-run view is [musique_independent100_candidate.json](../data/ids/musique_independent100_candidate.json); ordered-ID SHA-256 is `a026de236266ceb4ee7faa4cde5d031996ecfd74f4e6edd806a8b08cc493d42b`. The view records source and labels hashes. The read-only check covered 29 MuSiQue run manifests and 4 raw result files without manifests; there was no overlap. This range lies outside the already-used first 100 IDs and the unrun `pilot200` prefix, and does not use or alter the separately reserved `holdout100` at positions 400–499. Re-run the check immediately before any evaluation to include later artifacts.
