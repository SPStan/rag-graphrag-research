...
Последующая синтетическая проверка добавила gate-семантику для связанных triple refresh и третьей remedial attempt (не больше трёх попыток на stage), фильтрацию vector table по целевым IDs, merge attempt ledger без изменения исходных записей, patch OpenIE state и сборку hash-verified clone из готовых extraction и embedding outputs. Журнал сохраняет частичное восстановление при `finish_reason=length` только для распознанных структур без upstream parse error; статус остаётся `truncated`. Добавлен read-only планировщик и рассчитана точная очередь первого прохода: 38 NER и 158 triple запросов (38 — зависимые refresh, включая 36 дополнительных к 122 unresolved triple outcomes), всего 196; два запроса используют заранее разрешённую попытку №3. Это не верхняя граница при повторных ошибках. План содержит SHA manifest/corpus, hash очереди и только агрегаты: [hipporag-repair-plan.json](../results/summary/hipporag-repair-plan.json). В изолированном HippoRAG окружении без model requests отрисованы 38 NER и 120 известных triple prompts; ещё 38 triples зависят от будущих NER outputs. Отчёт [hipporag-repair-prompt-preflight.json](../results/summary/hipporag-repair-prompt-preflight.json) публикует только hashes и UTF-8 byte sizes, не token counts. Для точного Ollama Qwen-токенизатора локальный tokenizer-кэш не найден; установленная версия Ollama — 0.34.4, но tokenizer-backed context fit для фактических чатов этим не доказан. Добавлен executor одной задачи, использующий pinned OpenIE parser, обход response cache и отказ при наличии скрытых SDK retries; синтетические fake callbacks проверяют успешный пустой output, усечение и неизвестный usage. Добавлен crash-safe journal primitive: он фиксирует запрос как in-flight до вызова, записывает metadata после него, сверяет hash плана/source/protocol, отказывается от автоматического повтора прерванного запроса и запрещает сохранять prompt, passage, response или секреты. Пять синтетических тестов прошли; связка журнала с многозадачным runner ещё отсутствует. Ни одного нового model request или real index copy не было. Ordered orchestration, embedding request layer и graph integration не реализованы. Полный набор тестов: 130 passed; проверки `pip check` в этой серии не запускались. Текущая готовность и блокеры — в [preflight summary](../results/summary/hipporag-repair-preflight.json).
...
Добавлен `scripts/hipporag_repair_journal.py`: атомарный журнал до/после каждого потенциального repair-запроса. Он сверяет идентичность плана, исходных хэшей и frozen protocol; незавершённая in-flight задача блокирует автоматический resume, чтобы не отправить её повторно после сбоя. Запрещено сохранять prompts, passages, responses и секреты. Шесть тестов используют только временные синтетические данные. Весь unittest набор: 131 passed; model requests и индексные операции не выполнялись. Примитив пока не подключён к многозадачному runner, поэтому preflight остаётся `not_ready_for_model_calls`. Ollama 0.34.4 установлена, но точный tokenizer-backed context fit для фактически отрендеренных чатов не подтверждён.
