# История проверок стенда и экспериментов

Обновление 26 сентября: выполнено [диагностическое Dense / HippoRAG сравнение](COMPARISON_2026-09-26.md) на 100 одинаковых вопросах без ремонта индекса. F1 18,58% / 29,97%; строгий gate остаётся failed. Старые записи ниже описывают предыдущие этапы.

Записи ниже относятся к указанным датам. Актуальные ограничения — [TASKS](TASKS.md), замечания по текущему repair-коду — [ревью 25 сентября](REVIEW_2026-09-25.md). Предыдущий полный набор: 131 unittest и pip check основной .venv прошли; в этом документальном ревью полный набор не перезапускался.
...
`smoke_mlflow.py` печатает `verified` после записи, но пока сам не читает метрики обратно. Подтверждение чтения здесь основано на просмотре пользователем интерфейса. На 17 сентября это были следующие задачи; последующие проверки реальных run и рестартов описаны ниже.

## Ограничения проверки 17 сентября (исторические)
...
Read-only [repair readiness report](../results/summary/hipporag-repair-readiness-candidate.json) подтвердил сохранность OpenIE state (5 500 passages), графа и embedding stores: 5 500 passage, 44 614 entity и 47 614 fact vectors. Producer/embedding/OpenIE identity, SHA исходного корпуса и точные vector IDs совпадают с **текущим** OpenIE state. Это не доказывает безопасность будущей копии после исправлений. По журналу 38 NER и 122 triple terminal outcomes неуспешны; два passage затронуты обеими стадиями. Синтетические проверки теперь покрывают linked triple dependency refresh после NER correction и bounded remedial attempt 3 для двух unresolved stages. Read-only планировщик уточнил первую очередь: 38 NER и 158 triple запросов, включая 36 дополнительных refresh ранее валидных triples; всего 196 запланированных вызовов. Это не верхняя граница при повторных ошибках. Исходный индекс не менялся. Для исправленного OpenIE state нужно создать в копии store с **точным новым набором** entity/fact IDs: переиспользовать совпадающие векторы, добавить новые и убрать устаревшие до пересборки графа. Иначе upstream использует все IDs в store при построении synonymy edges ([pinned source](https://github.com/OSU-NLP-Group/HippoRAG/blob/1438aba3fc44ff10573e5a5e1e7cc3c7f9794aff/src/hipporag/HippoRAG.py#L1143-L1182)). Ремонт всё ещё не готов к запуску; см. [ADR-0008](../.adr/0008-openie-truncation-retry-policy.md).

На этапе подготовки repair синтетически проверены linked triple refresh, bounded attempt 3, merge ledger, state patch, vector reconciliation и copy-only assembler. Read-only план: 38 NER + 158 triples = 196 задач первого прохода, включая 38 зависимых refresh; [хэш и агрегаты очереди](../results/summary/hipporag-repair-plan.json). Это не полный бюджет повторов.

Pinned template preflight отрендерил 158 известных prompts; 38 зависят от будущего NER. [Отчёт](../results/summary/hipporag-repair-prompt-preflight.json) содержит SHA и UTF-8 byte sizes, не token counts; context fit не подтверждён. Single-task executor проверен с fake callbacks. Последующие 131 unittest включают шесть тестов журнала, но не целостный real repair.

Текущий [preflight](../results/summary/hipporag-repair-preflight.json) остаётся `not_ready_for_model_calls`. Аудит исходников и синтетическое воспроизведение показали ограничения восстановления outputs, нескольких writer и task fields; см. [F1–F6](REVIEW_2026-09-25.md). Проверки отдельных функций не означают готовность отремонтированного графа.
...
Синтетические тесты protocol helper и заморозки выборки проверяют статусы, gate, retry linkage, стабильность ordered IDs и отказ при пересечении. После исправления retry и уточнения summary: 97 unittest passed; основная `.venv` проходит `pip check`. До запуска candidate read-only audit проверил 29 MuSiQue manifests и 4 raw JSONL без manifests, пересечений не было. Candidate запуск описан в разделе выше; последующие изменения не создавали новых MLflow/Langfuse записей, LLM-вызовов или индексов.

## Crash-safe repair journal primitive, 25 сентября 2026

Добавлен `scripts/hipporag_repair_journal.py` с записью in-flight и проверкой порядка задач. Шесть исходных синтетических тестов прошли в предыдущем шаге; весь набор тогда составил 131 тест, pip check основной .venv прошёл.

В текущем ревью без модельных вызовов на временных синтетических файлах подтверждены три ограничения: успешные values не сохраняются API журнала, два заранее открытых writer допускают begin одной задачи, поле prompt в task попадает на диск. Поэтому обещания полного resume и исключения любого текста из журнала преждевременны. Это не свидетельство утечки реальных данных: проверка использовала только строку SYNTHETIC_ONLY. Код исправляется в R1, результаты и реальные индексы аудитом не менялись.
