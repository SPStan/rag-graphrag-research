...
Последующая синтетическая проверка добавила gate-семантику для связанных triple refresh и третьей remedial attempt (не больше трёх попыток на stage), а также чистую фильтрацию vector table с отчётом о новых и устаревших IDs. Журнал теперь сохраняет частичное восстановление при `finish_reason=length` только для распознанных структур без upstream parse error; статус остаётся `truncated`. Это подготовительные функции, не repair orchestrator: копирование индекса, импорт реальной истории, добавление новых embeddings и пересборка графа ещё не реализованы и не запускались. Полный набор unit tests: 107 passed; `pip check`: passed.
