...
Последующая синтетическая проверка добавила gate-семантику для связанных triple refresh и третьей remedial attempt (не больше трёх попыток на stage), фильтрацию vector table по целевым IDs, merge attempt ledger без изменения исходных записей, patch OpenIE state и сборку hash-verified clone из готовых extraction и embedding outputs. Журнал сохраняет частичное восстановление при `finish_reason=length` только для распознанных структур без upstream parse error; статус остаётся `truncated`. Добавлен read-only планировщик и рассчитана точная очередь первого прохода: 38 NER запросов и 158 triple запросов (из них 38 обновляют triples с учётом исправленных entities), всего 196; два запроса используют заранее разрешённую попытку №3. Это план, не верхняя граница возможной стоимости. План содержит SHA manifest/corpus, hash очереди и только агрегаты: [hipporag-repair-plan.json](../results/summary/hipporag-repair-plan.json). Ни одного нового model request не было. Helper клонирования не запускался на реальном индексе; cache-bypass extraction/embedding runner и его manifest wiring, проверка prompt token budget и графовая интеграция ещё не реализованы. Все 116 unit tests passed; `pip check` проходит. Готовность и блокеры — в [preflight summary](../results/summary/hipporag-repair-preflight.json).
