...
Read-only [repair readiness report](../results/summary/hipporag-repair-readiness-candidate.json) подтвердил сохранность OpenIE state (5 500 passages), графа и embedding stores: 5 500 passage, 44 614 entity и 47 614 fact vectors. По журналу 38 NER и 122 triple terminal outcomes неуспешны; два passage затронуты обеими стадиями. Если корректировка NER требует обновить зависимые triples, предварительная оценка — 196 точечных LLM-generation calls, а не повторная индексация 5 500 passages. Это оценка объёма запросов, не гарантия времени или успеха. Нужны проверка совместимости и реализация клонирования; source index не изменялся, ни одной модели не вызывали. Граф следует перестроить только в копии с повторным использованием проверенных stores; upstream поддерживает отдельный `force_index_from_scratch` для сборки графа с совместимыми сохранёнными embedding/OpenIE stores ([pinned HippoRAG index implementation](https://github.com/OSU-NLP-Group/HippoRAG/blob/1438aba3fc44ff10573e5a5e1e7cc3c7f9794aff/src/hipporag/HippoRAG.py#L493-L535), [embedding store deduplicates existing vectors](https://github.com/OSU-NLP-Group/HippoRAG/blob/1438aba3fc44ff10573e5a5e1e7cc3c7f9794aff/src/hipporag/embedding_store.py#L117-L136)).
