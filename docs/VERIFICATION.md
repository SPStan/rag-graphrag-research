...
Последующая синтетическая проверка добавила gate-семантику для связанных triple refresh и третьей remedial attempt (не больше трёх попыток на stage), фильтрацию vector table по целевым IDs, merge attempt ledger без изменения исходных записей, patch OpenIE state и сборку hash-verified clone из готовых extraction и embedding outputs. Журнал теперь сохраняет частичное восстановление при `finish_reason=length` только для распознанных структур без upstream parse error; статус остаётся `truncated`. Helper клонирования не запускался на реальном индексе; runner для новых extraction/embedding запросов, wiring run manifest и графовая интеграционная проверка пока не реализованы. Полный набор unit tests: 113 passed; `pip check`: passed. Готовность и оставшиеся блокеры перечислены в [preflight summary](../results/summary/hipporag-repair-preflight.json).
