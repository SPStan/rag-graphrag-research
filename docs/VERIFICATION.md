...
HippoRAG run `e78eff08-532a-40b3-a359-49a6b08b32a7` использовал новый storage namespace и отдельный manifest. Индексный build завершился, но acceptance gate отклонил индекс до QA: из 11 000 ожидаемых terminal stage outcomes осталось 160 unresolved (122 triples truncation, 37 NER truncation, 1 NER request error); missing outcomes — 0, unexpected attempts — 0, записано 11 025 попыток. В NER было 5 498 кэш-попаданий, 24 miss и 2 неатрибутированных на LLM-layer случая; две записи usage неизвестны. В журнале попыток также есть 21 parse-error попытка NER, часть не является terminal outcome после retry. Суммарный известный local Ollama usage index build: 7 240 351 prompt и 1 522 360 completion tokens; это локальные счётчики, не внутренний API. Сборка заняла 14 427,9 секунды. QA results отсутствуют, поэтому парное сравнение и bootstrap не выполнялись. Index и кэши сохранены как диагностические; автоматический retry/rebuild не запускался.
...
Синтетические тесты protocol helper и заморозки выборки проверяют статусы, gate, retry linkage, стабильность ordered IDs и отказ при пересечении. Последняя проверка после реализации протокола: 96 unittest passed; основная `.venv` и временная HippoRAG `.venv` проходят `pip check`. До запуска candidate read-only audit проверил 29 MuSiQue manifests и 4 raw JSONL без manifests, пересечений не было. Candidate запуск описан в разделе ниже; последующие doc-only обновления не создавали новых MLflow/Langfuse записей, LLM-вызовов или индексов.
