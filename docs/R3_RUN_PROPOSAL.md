...
Счётчик completion этих двух измерений не был сохранён; он неизвестен, а не равен нулю. Пилот не доказывает токены остальных prompts и не подтверждает совпадение native `/api/chat` с фактическим OpenAI-compatible запросом. Текущий guard остаётся закрытым.
