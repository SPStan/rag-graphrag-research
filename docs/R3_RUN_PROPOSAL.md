...
Исторические команды уже выполненного запуска (не повторять): offline-проверка `& $hippoPython -m scripts.run_hipporag_repair --remedial-ner --dry-run`; ограниченный запуск `& $hippoPython -m scripts.run_hipporag_repair --remedial-ner --execute`.

**Фактический итог:** Luna выполнила ровно одно измерение (334 input / 1 completion) и одну extraction (334 input / 2048 completion) на коммите `99defab`. Attempt 3 снова `truncated`; новый checkpoint `stopped/unresolved_extraction`, SHA `1cf000239921f3dc36d4a879e5defe355c8ecbb7afc7e092bc31a193c8c57275`. Исходный checkpoint остался с SHA `83f61962940c06c912ff0c595a8617be298698b802151bc50d8136d2ef331d13`. Команду не повторять. Значения частичного JSON не приняты, очередь из 196 задач не продолжалась, embeddings/rebuild/QA не запускались. Дальнейшее повышение cap или attempt 4 не разрешены; необходим новый методологический выбор по ADR-0008.
